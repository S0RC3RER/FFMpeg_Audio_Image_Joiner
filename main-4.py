...
def combine_video(image_file, audio, output_file):
    """
    ...
    ffmpeg.output(inputs, output_file, c="libx264", tune="stillimage", a="aac", video_bitrate=192000, audio_bitrate=192000, pix_fmt="yuv420p",
                  short="shortest").run()
...
class Application(tk.Frame):
    ...
    def combine_command(self):
        ...
        if not output_file:
            return
        self.progress.start()
        combine_video(self.image_file, audio_files, output_file)
        self.progress.stop()
